 ...
 *
 * Colours are the same three this page's table uses, so a red block in the chart
 * and a red row below it mean the same thing. Validated against the light surface:
 * accent vs revoked reads at ΔE 15.5 under protanopia and 24.8 to normal vision,
 * both above the floor. The neutral for duplicates is deliberately outside that
 * pair — it is a status neutral, not a third identity, and it is the one segment
 * nobody needs to distinguish at a glance.
 */
const SERIES = [
  { key: "valid", label: "Valid", fill: "var(--color-accent)" },
  { key: "duplicate", label: "Duplicate", fill: "var(--color-rule-strong)" },
        ...
        <ul className="flex flex-wrap items-center gap-4">
          {SERIES.map((series) => (
            <li key={series.key} className="flex items-center gap-1.5">
              ...
              />
              <span className="text-xs text-ink-700">{series.label}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="relative mt-4">
        ...
        <div className="mt-1.5 flex">
          {columns.map((column, index) => (
            <span
              key={column.hour}
              className="serial shrink-0 text-center text-[10px] text-ink-500 tabular-nums"
        ...
        {hovered !== null ? (
          <div
            className="pointer-events-none absolute -top-1 rounded-md border border-rule bg-card px-3 py-2 shadow-sm"
            style={{
              left: `${Math.min(Math.max(hovered * barWidth, 0), 78)}%`,
            }}
          >
            <p className="serial text-[11px] font-semibold text-ink-900">
            ...
            {SERIES.map((series) => (
              <p key={series.key} className="mt-0.5 flex items-center gap-1.5 text-[11px]">
                <span
                  ...
                  style={{ background: series.fill }}
                />
                <span className="text-ink-500">{series.label}</span>
                <span className="serial ml-auto pl-2 font-medium text-ink-900 tabular-nums">
